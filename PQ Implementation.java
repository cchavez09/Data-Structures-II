 ...
 * nomatter where it is on the queue pefore it gets processes (popped off), and 
 ...
 * acts as a handle to the queued object when it is on the priorityy queue. The 
...
class PriorityQueue<E, P> {
    ...
    public Node add(E e, P priority) {
        ...
        // YOUR CODE GOES HERE / ADD YOU NAME AT TOP OF FILE
      // create a new Node with the parameters as well as the tree size 
      Node newNode = new Node(e, priority, tree.size());
      // add the new node to the tree, goes to right most leaf on lowest level
      tree.add(newNode);
      // call the bubbleUp method to move the new node up the tree
    ...
    public boolean contains(E e) {
      ...
      // ADD YOUR CODE HERE
      // check if tree size is greater than 1, if not return false
      if (tree.size() > 1) {
        // iterate through arrayList which is tree
        for (int num = 0; num < tree.size(); num++) {
          // check if the index of the element is equal to the element
          if (tree.get(num).value.equals(e)){
            // if yes, return true
            return true;
          }
        }
 ...
 * Main Routine
 *
 * The  routine is simply exercising the methods, it is not performing
 * unit testing. It is only illustrating method use.
 *
 *********************************************************************/
...
public class Main {
    public static void main(String[] args) {
        ...
        System.out.println("Starting Priority Queue testing execution!");
        PriorityQueue<String, Integer> pq = new PriorityQueue<>();
        PriorityQueue<String, Integer>.Node element;

        /*
         * Initial queue population, utilizing both add() and offer().
         */
        ...
        /*
         * Display the head of the queue (highest priority element),
         * and show use of the method contains(). Then finally print out 
         * the full queue. The min heap tree representation is flattened 
         * on the tree print out.
         */

        System.out.println("\nRunning test 1...");

        if (pq.peek().value() != "element5" && pq.peek().priority() != 5)
            System.out.println("   TEST 1 FAILED!!!");

        if (! pq.contains("element3"))
            System.out.println("  Test 1, 1st Contains FAILED");

        if (pq.contains("NOT IN QUEUE"))
            System.out.println("  Test 1, 2nd Contains FAILED");

        pq.printPriorityQueue();

        /*
         * Add element, then change it's priority after adding.
         * In this case, we have a handle to the element when it is on
         * the queue. And, after changing priority, it will be repositioned
         * in the min heap.
         */

        element = pq.add("element6", 45);
        element.changePriority(2);

        System.out.println("\n\nRunning test 2...");

      if (pq.peek().value() != "element6" && pq.peek().priority() != 2)
        System.out.println("   TEST 2 FAILED!!!");
      


        /*
         * Now change the element we have a handle too, which is the head element 
         * (highest priority) to a lower priority. It should be repositioned in 
         * the min heap as a result.
         */
      
        element.changePriority(35);
        System.out.println("\n\nRunning test 3...");
        System.out.println("Should dequeue in this order: elements 5, 1, 2, 3, 4, and 6");
                           

        System.out.println("\nDequeuing all elements in priority order...");
        while(!pq.isEmpty()) {
            element = pq.remove();
            System.out.println(String.format("Dequeuing value: %s priority: %d", 
                                              element.value(), element.priority()));
        }

        /*
         * Queue is empty, now add more elements
         */
        ...
        System.out.println("Queue shoudl be [ (ghi,1), (abc,5), ]");
        pq.printPriorityQueue();

        if ( pq.isEmpty() )
            System.out.println("ERROR - queue is empty and should not be");

        pq.clear();

        if ( ! pq.isEmpty() )
            System.out.println("ERROR - queue is not empty and should be");

        System.out.println("\nCleared queue, now printing it, should be empty");
        pq.printPriorityQueue();

        System.out.println("\n\nFinished Priority Queue testing execution!");
    }
}